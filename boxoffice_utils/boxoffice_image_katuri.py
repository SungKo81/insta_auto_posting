...
# 인스타 이미지 생성 함수
def insta_boxoffice_katuri(title, watches, tot_watches, rank):
    try:
        # 포스터 열기
        ...
        save_as = os.path.join(output_path, f'No0{10-rank} {now}_insta_{image_name}.jpg')
        poster.save(save_as)
...
# 주간순위 리스트 페이지 만드는 함수 
def make_list_page_katuri(movies, page):
    # 폴더 내 이미지 파일 목록 가져오기
